...
| `columns` | `ColumnDef<TablecraftFeatures, TData, any>[]` | required | Column definitions |
...
|---|---|---|
| `table` | `TablecraftTable<TData>` | Full TanStack Table instance with all accumulated rows |
...
- `useInfiniteTable` does not get `columnOrder`, `rowPinning` or `columnResizing` — its return type is standalone rather than extending `useTable`'s, so these three were left out of scope deliberately. If you need them, use [`useQueryTable`](/docs/api/use-query-table) or [`useTable`](/docs/api/use-table) instead, both of which support all three with the identical option shape.
