...
| `columnOrder` | `ColumnOrderOptions \| boolean` | — | Opt-in column ordering |
| `rowPinning` | `RowPinningOptions \| boolean` | — | Opt-in row pinning |
| `columnResizing` | `ColumnResizingOptions \| boolean` | — | Opt-in column resizing — `mode` defaults to `'onEnd'` |
| `fuzzy` | `boolean \| FilterFn<TablecraftFeatures, TData>` | — | Same caveat as `useTable` — `true` requires CJS/Node |
